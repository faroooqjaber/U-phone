...
def setup_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS settings (guild_id INTEGER PRIMARY KEY, app_channel INTEGER, tweet_channel INTEGER, verify_channel INTEGER, admin_channel INTEGER, embed_color TEXT)''')
...
class TweetModal(ui.Modal, title="إرسال تغريدة جديدة"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        setting = query_db("SELECT tweet_channel, embed_color FROM settings WHERE guild_id = ?", (interaction.guild.id,), one=True)
...
class VerifyModal(ui.Modal, title="تقديم طلب توثيق الحساب"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        setting = query_db("SELECT verify_channel FROM settings WHERE guild_id = ?", (interaction.guild.id,), one=True)
        ...
        await v_chan.send(embed=embed, view=VerifyActionView(interaction.user.id))
...
# ==========================================
# 🔘 واجهات الأزرار والتفاعلات (Views)
# ==========================================
...
class MainPanelView(ui.View):
    ...
    @ui.button(label="إنشاء حساب 📝", style=discord.ButtonStyle.green, custom_id="btn_register")
    async def reg(self, interaction: discord.Interaction, button: ui.Button):
        await interaction.response.send_modal(RegisterModal())
        
    @ui.button(label="إرسال تغريدة 🐦", style=discord.ButtonStyle.blurple, custom_id="btn_tweet")
    async def tw(self, interaction: discord.Interaction, button: ui.Button):
        await interaction.response.send_modal(TweetModal())
        
    @ui.button(label="طلب توثيق 🏅", style=discord.ButtonStyle.secondary, custom_id="btn_verify")
    async def vf(self, interaction: discord.Interaction, button: ui.Button):
        await interaction.response.send_modal(VerifyModal())
        
    @ui.button(label="حسابي 👤", style=discord.ButtonStyle.primary, custom_id="btn_my_acc")
    async def my_acc(self, interaction: discord.Interaction, button: ui.Button):
        ...
        top_tags = query_db("SELECT tag, count FROM hashtags ORDER BY count DESC LIMIT 3")
        ...
        embed.add_field(name="📈 ترندات المدينة الأكثر تداولاً:", value=trend_text, inline=False)
        
        await interaction.response.send_message(embed=embed, ephemeral=True)

    @ui.button(label="البحث عن حساب 🔍", style=discord.ButtonStyle.secondary, custom_id="btn_search")
    async def src(self, interaction: discord.Interaction, button: ui.Button):
...
async def on_ready():
    ...
    # إعادة ربط الـ Views الثابتة المستمرة لضمان عمل الأزرار بعد ريستارت البوت
...
async def help_cmd(interaction: discord.Interaction):
    embed = discord.Embed(
        ...
        description="نظام تويتر المتكامل لمدينة يو ار جي:\n\n"
                    "1️⃣ **روم التطبيق:** يحتوي على الأزرار لفتح النوافذ المنبثقة والتحكم بملفك.\n"
                    "2️⃣ **إنشاء الحساب:** يتيح لك حجز يوزر نيم فريد لا يتكرر بالمدينة.\n"
                    "3️⃣ **التغريد:** يدعم الهاشتاقات والمرفقات (روابط صور فقط، الروابط الأخرى محظورة).\n"
                    "4️⃣ **التفاعلات:** التغريدات تظهر بأزرار (أعجبني، ريتويت، وردود تفتح داخل ثيرد مستقل).\n"
                    "5️⃣ **المتابعة:** ابحث عن أي شخص وتابعه لرفع إحصائيات حسابه!",
        color=discord.Color.blue()
    )
    await interaction.response.send_message(embed=embed, ephemeral=True)

@bot.tree.command(name="تسطيب_البوت", description="إعداد وتجهيز منظومة تويتر رول بلي (للإدارة)")
@app_commands.checks.has_permissions(administrator=True)
async def setup_cmd(interaction: discord.Interaction, روم_التطبيق: discord.TextChannel, روم_التغريدات: discord.TextChannel, روم_طلبات_التوثيق: discord.TextChannel, روم_الادارة: discord.TextChannel, لون_الامبد: str = "00acee"):
    
    query_db('''INSERT OR REPLACE INTO settings (guild_id, app_channel, tweet_channel, verify_channel, admin_channel, embed_color) 
              VALUES (?, ?, ?, ?, ?, ?)''', 
              (interaction.guild.id, روم_التطبيق.id, روم_التغريدات.id, روم_طلبات_التوثيق.id, روم_الادارة.id, لون_الامبد), commit=True)

    panel_embed = discord.Embed(
        title="📱 تطبيق U app | اللوحة الرئيسية",
        description="مرحباً بك في منصة تويتر الرسمية المخصصة لسكان مدينة يو ار جي.\n\n"
                    "يمكنك التحكم التام بحسابك والتواصل البرمي المميز من خلال الأزرار التفاعلية بالأسفل 👇",
    ...
    await interaction.response.send_message("✅ تم إعداد القنوات وحفظ البيانات بنجاح، وإرسال البانل الرئيسي واللوحة الإدارية!", ephemeral=True)
